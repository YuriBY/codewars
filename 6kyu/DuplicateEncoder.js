...
function duplicateEncode(word){
    const wordSimilarCase = word.toLowerCase();
    let newStr = '';
    let dublicateChars = [];
    for (let i=0; i < wordSimilarCase.length; i++) {
        if (dublicateChars.includes(wordSimilarCase[i])) {
            newStr += ')';
        } else {
            if (wordSimilarCase.indexOf(wordSimilarCase[i], i + 1) > 0) {
                dublicateChars.push(wordSimilarCase[i]);                
                newStr += ')';
            } else {
                newStr += '(';
            }
